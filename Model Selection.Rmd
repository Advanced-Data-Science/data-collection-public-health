...
pacman::p_load(readxl, dplyr, lubridate, ggplot2, scales, ggcorrplot, maps, rpart, rpart.plot, caret)
...
Data <- read_excel("/Users/madelynnjoan/Desktop/FDAcleaned.xlsx") %>% 
  ...
  ##seasons variable
      mutate(recall_date = mdy(recall_date), # convert to proper Date
              month = month(recall_date),   # extract month number (1–12)
...
```{r model framework and metric}
nrow(Data)
ncol(Data)

# Predict using the pruned tree
pred_class <- predict(pruned, newdata = Data, type = "class")

# Confusion matrix + F1
cm <- confusionMatrix(pred_class, Data$ClassI, positive = "Yes", mode = "everything")

cm      # full confusion matrix output
cm$byClass["F1"]   # just the F1 score
